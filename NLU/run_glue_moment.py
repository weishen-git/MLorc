...
from optim import MLorc_AdamW, GaLore
...
def parse_args():
    ...
    parser.add_argument(
        "--warmup_ratio", type=float, default=0.03, help="Ratio of the warmup steps in the lr scheduler."
    )
...
def main():
    ...
    if args.op=='Full':
        ...
        for name, param in model.named_parameters():
            if any(key in name for key in target_modules_list) and param.dim()== 2:
                para.append(param)

        if args.op=='MLorc_AdamW':
            optimizer = MLorc_AdamW(para, lr=args.learning_rate, rank=args.lora_r)
        if args.op=='GaLore':
            optimizer = GaLore(para, lr=args.learning_rate, rank=args.lora_r, T=args.update_proj_gap)
    ...
    his_singular_values=[]
    his_m_singular_values=[]
    his_sq_singular_values=[]
    completed_steps=0
    names=[]
                ...
                if completed_steps%10==1:
                    ...
                    with torch.no_grad():
                        ...
                        for name, param in model.named_parameters():
                            ...
                            if param.grad.dim() != 2:
                                continue
                            if not any(key in name for key in target_modules_list):
                                continue
                            ...
                            m = state['exp_avg']       
                            sq = state['exp_avg_sq']    
                            m_float=m.to(torch.float32)
                            sq_float=sq.to(torch.float32)
                            _, m_s, _=torch.linalg.svd(m_float)
                            _, sq_s, _=torch.linalg.svd(sq_float)
                            
                            m_s=m_s.cpu().numpy()
                            sq_s=sq_s.cpu().numpy()
                            m_singular_values.append(m_s)
                            sq_singular_values.append(sq_s)

                            grad_float=param.grad.to(torch.float32)
                            _, s, _=torch.linalg.svd(grad_float)
                            s=s.cpu().numpy()
                            singular_values.append(s)
                        his_singular_values.append(singular_values)
                        his_m_singular_values.append(m_singular_values)
                        his_sq_singular_values.append(sq_singular_values)
    ...
    with open(os.path.join(args.output_dir, "memory.json"), "w") as f:
    ...
    with open(os.path.join(args.output_dir, "runtime.txt"), 'w') as f:
        f.write(f"Runtime: {elapsed_time:.6f} s\n")

    import pickle


    with open(f'{args.output_dir}/m_singular_values.pkl', 'wb') as f:
        pickle.dump(his_m_singular_values, f)
    with open(f'{args.output_dir}/sq_singular_values.pkl', 'wb') as f:
        pickle.dump(his_sq_singular_values, f)
    with open(f'{args.output_dir}/singular_values.pkl', 'wb') as f:
        pickle.dump(his_singular_values, f)


    with open(f'{args.output_dir}/names.txt', "w") as f:
       for item in names:
           f.write(item + "\n")
