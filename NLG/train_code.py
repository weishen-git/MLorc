...
from Preprocessing import load_codefeedback, CodeFeedback10k_Preprocessor
...
def main():
  ...
  if local_rank == 0:
        wandb.init(
            project='LLAMA-2-7B',
            name=f"llama-2-7b_code_{config['optimizer']}_LoRA_{config['LoRA']}",
  ...
  with TitledLog("load datasets and dataloaders", log_fn=log.info):
      ...
      preprocessor = CodeFeedback10k_Preprocessor(
            tokenizer=tokenizer,
            tokenizer_kwargs={
                "padding": "max_length",
                "truncation": True,
  ...
  if local_rank == 0:
      ...
      model.save_pretrained(f'./logs/transformers/llama-2-7b/code/LoRA_{config['LoRA']}/optimizer_{config["optimizer"]}/lr_{config["learning_rate"]}')
      tokenizer.save_pretrained(f'./logs/transformers/llama-2-7b/code/LoRA_{config['LoRA']}/optimizer_{config["optimizer"]}/lr_{config["learning_rate"]}')
