...
def main():
    ...
    if config['LoRA'] == "On":
        ...
        model = PeftModel.from_pretrained(model, f'./logs/transformers/llama-2-7b/code/LoRA_{config['LoRA']}/optimizer_{config["optimizer"]}/lr_{config["learning_rate"]}')
        model = model.to(dtype=torch.bfloat16)
    else:
        model = transformers.LlamaForCausalLM.from_pretrained(f'./logs/transformers/llama-2-7b/code/LoRA_{config['LoRA']}/optimizer_{config["optimizer"]}/lr_{config["learning_rate"]}', max_length=1024, attn_implementation="flash_attention_2", torch_dtype=torch.bfloat16, device_map={"": int(os.environ.get("LOCAL_RANK") or 0)}, use_auth_token=True)
    model.config.use_cache = True
    model.gradient_checkpointing_disable()
    tokenizer = transformers.LlamaTokenizer.from_pretrained(f'./logs/transformers/llama-2-7b/code/LoRA_{config['LoRA']}/optimizer_{config["optimizer"]}/lr_{config["learning_rate"]}', padding_side="left")
